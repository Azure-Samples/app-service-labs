...
The App Service managed-certificate CLI command is currently in preview. Create
the certificate, capture its thumbprint, and bind it to the hostname:

```bash
CERT_THUMBPRINT=$(az webapp config ssl create \
  --name "$APP_NAME" \
  --resource-group "$RG_NAME" \
  --hostname "$CUSTOM_HOSTNAME" \
